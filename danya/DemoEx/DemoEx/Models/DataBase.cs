...
namespace DemoEx.Models
{
    ...
    {
        private static string connectionString = "Data Source=BAYRONT\\SQLEXPRESS;Trusted_Connection=Yes;DataBase=SessionEx;";
        ...
        public static void RegUser(User user)
        {
            ...
            {
                connection.Open();
                string query = "INSERT INTO Users (Email, Pssword, FirstName, LastName, RoleID) VALUES (@Email, @Pssword, @FirstName, @LastName, 2)";
                SqlCommand command = new SqlCommand(query, connection);
        ...
        public static string ValidateUser(string email, string password)
        {
            ...
            {
                connection.Open();
                string query = "SELECT RoleID FROM Users WHERE Email = @Email AND Pssword = @Pssword";
                SqlCommand command = new SqlCommand(query, connection);
                command.Parameters.AddWithValue("@Email", email);
                command.Parameters.AddWithValue("@Pssword", hashedPassword); // Сравниваем с хэшем пароля из базы данных

                object result = command.ExecuteScalar();
                if (result != null)
                {
                    int roleId = (int)result;
                    string roleQuery = "SELECT Title FROM Roles WHERE ID = @RoleID";
